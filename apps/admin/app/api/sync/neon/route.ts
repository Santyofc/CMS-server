...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkRateLimit(`sync-neon:${auth.user.id}`, 10, 60_000);
  if (!rate.allowed) {
    return NextResponse.json({ error: "Too many requests" }, { status: 429 });
  }

  try {
    const data = await syncNeon();
    ...
    await writeAuditLog({
      ...
      errorMessage: data.status === "success" ? undefined : "Neon sync returned failed status"
    });
