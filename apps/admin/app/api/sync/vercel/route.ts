...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkRateLimit(`sync-vercel:${auth.user.id}`, 10, 60_000);
  if (!rate.allowed) {
    return NextResponse.json({ error: "Too many requests" }, { status: 429 });
  }

  try {
    const data = await syncVercel();
    ...
    await writeAuditLog({
      ...
      errorMessage: data.status === "success" ? undefined : "Vercel sync returned failed status"
    });
