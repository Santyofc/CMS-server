...
export async function POST(request: NextRequest) {
  ...
  const rate = await checkRateLimit(`sync-spaceship-dns:${auth.user.id}`, 10, 60_000);
  if (!rate.allowed) {
  ...
  try {
    ...
    await writeAuditLog({
      ...
      errorMessage: data.status === "success" ? undefined : "Spaceship DNS sync returned failed status"
    });
    return NextResponse.json({ data }, { status: statusCode });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Spaceship DNS sync failed";
    await writeAuditLog({
      userId: auth.user.id,
      action: "sync_spaceship_dns",
      provider: "spaceship",
